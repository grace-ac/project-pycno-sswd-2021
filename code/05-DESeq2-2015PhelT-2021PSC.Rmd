...
Make a data frame for the comparison: 
```{r}
colData2 <- data.frame(condition=factor(c("healthy","healthy","healthy","healthy","healthy", "healthy", "healthy" ,"healthy", "sick","sick","sick","sick", "sick", "sick", "sick", "sick")),
                      type=factor(rep("paired-end",16)))
rownames(colData2) <- colnames(counts_hvs2)
dds2 <- DESeqDataSetFromMatrix(countData = counts_hvs2,
                              colData = colData2,
                              design = ~ condition)
```
```{r}
...
hvs_fig2 <- res2
# The main plot
plot(hvs_fig2$baseMean, hvs_fig2$log2FoldChange, pch=20, cex=0.45, ylim=c(-15, 15), log="x", col="darkgray",
     main="Healthy vs Sick  (pval </= 0.05)",
     xlab="mean of normalized counts",
     ylab="Log2 Fold Change")
# Getting the significant points and plotting them again so they're a different color
hvs_fig.sig2 <- res2[!is.na(res2$padj) & res2$padj <= 0.05, ]
points(hvs_fig.sig2$baseMean, hvs_fig.sig2$log2FoldChange, pch=20, cex=0.45, col="red")
# 2 FC lines
abline(h=c(-1,1), col="blue")
```
...
Write out DEG file: 
```{r}
#write.table(hvs_fig.sig2, "../analyses/DESeq2/2015Phel/DEGlist_healthy-vs-sick_8x8.tab", sep = "\t", row.names = T, quote = FALSE, col.names = TRUE)
```
HUGE file
wrote out 20221204
...
# Try to get number of hits with adj-p-value less than 0.05; as well as those with log2FC >2.00 and <-2.00
# get those greater than 2.00 log2FC
hvs_fig.sig3 <- subset.data.frame(hvs_fig.sig2, hvs_fig.sig2$log2FoldChange > 2.00)
# get those less than -2.00 log2FC
hvs_fig.sig4 <- subset.data.frame(hvs_fig.sig2, hvs_fig.sig2$log2FoldChange < -2.00)
# combine the two into 1 list of DEGs with p-values </= 0.05 and log2FC >2.00 and <-2.00
hvs_fig.sig.final <- rbind(hvs_fig.sig3, hvs_fig.sig4)
...
Write out DEG file: 
```{r}
write.table(tvs_fig.sig, "../analyses/DESeq2/2015Phel/DEGlist_tankexposed_v_sick_4x4.tab", sep = "\t", row.names = T, quote = FALSE, col.names = TRUE)
```
wrote out 20221204
